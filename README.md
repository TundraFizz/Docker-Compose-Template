...
# [NEW] Adding/Updating a service

========== FIRST TIME SETUP ==========

Make sure that a directory with the correct service name has been created in the `docker` directory
...
Run `clear; npm run deploy_backend` on your local environment

Once the service is accessible on DOMAIN_URL, create an SSL certificate with Let's Encrypt, modify the NGINX config files, and restart NGINX
```
docker run -it --rm --name certbot                      \
-v tundra_ssl:/etc/letsencrypt                          \
-v tundra_ssl_challenge:/ssl_challenge                  \
...
# [OLD] Adding a new service (Domain name with SSL)
